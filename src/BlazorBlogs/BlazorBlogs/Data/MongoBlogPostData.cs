...
public class MongoBlogPostData : IBlogPostData
{
	...
	{
		FilterDefinition<BlogPost>? filter = Builders<BlogPost>.Filter.Eq("Id", post.Id);
	...
	{
		IAsyncCursor<BlogPost>? results = await _posts.FindAsync(_ => true);
	...
	{
		IAsyncCursor<BlogPost>? results = await _posts.FindAsync(u => u.Url == url);
